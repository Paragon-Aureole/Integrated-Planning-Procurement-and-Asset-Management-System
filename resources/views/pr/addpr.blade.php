            ...
            <div class="row">
              <div class="form-group col-md-12">
                ...
                <select class="custom-select custom-select-sm" name="pr_code" readonly required>
                  ...
                  $prcode = "PR-".$office_ppmp->office_code.'-'.date('Y').'-'.sprintf('%02d',
                  Auth::id()).'-'.sprintf('%04d', $pr_query);
                  @endphp
                  <option @if($requestor !=null)data-requestor-id="{{$requestor->id}}"
                    data-requestor-name="{{$requestor->signatory_name}}" @endif data-office-id="{{$office_ppmp->id}}"
                    data-office-name="{{$office_ppmp->office_name}}" value="{{$prcode}}">{{$prcode}}</option>
                  @endforeach
                </select>
                @else
                @php
                $prcode = "PR-".$user->office->office_code.'-'.date('Y').'-'.sprintf('%02d',
                Auth::id()).'-'.sprintf('%04d', $pr_count);
                @endphp
              ...
              <div class="form-group col-md-12">
                ...
                @else
                <input class="form-control form-control-sm " type="text" value="{{Auth::user()->office->office_name}}"
                  disabled>
              ...
              <div class="form-group col-md-12">
                ...
                <textarea id="prPurpose" name="pr_purpose"
                  class="form-control form-control-sm {{ $errors->has('pr_purpose') ? 'is-invalid' : '' }}" rows="3"
                  required>{{old('pr_purpose')}}</textarea>
                <div class="invalid-feedback">
              ...
              <div class="form-group col-md-6">
                ...
                <select id="suppplierType"
                  class="custom-select custom-select-sm {{ $errors->has('supplier_type') ? 'is-invalid' : '' }}"
              ...
              <div class="form-group col-md-6" id="supplierId">
                ...
                <select class="custom-select custom-select-sm {{ $errors->has('supplier_id') ? 'is-invalid' : '' }}"
                  name="supplier_id">
              ...
              <div class="form-group col-md-6" id="agencyName">
                ...
                <input class="form-control form-control-sm {{ $errors->has('agency_name') ? 'is-invalid' : '' }}"
                  name="agency_name" value="{{old('agency_name')}}">
                <div class="invalid-feedback">
              ...
              <div class="form-group col-md-12">
                ...
                <input id="requestorName"
                  class="form-control form-control-sm {{ $errors->has('pr_requestor') ? 'is-invalid' : '' }}" value=""
                  disabled>
                ...
                ->first();
                @endphp
                <input class="form-control form-control-sm {{ $errors->has('pr_requestor') ? 'is-invalid' : '' }}"
                  type="text" value="{{$requestor->signatory_name}}" disabled>
